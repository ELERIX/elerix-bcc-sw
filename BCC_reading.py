...
def error_message(value, onlywrite = 0):
    ...
    if value == 0:
        ...
        mess += ".\nCannot download from any BCC, waiting for connection."
    elif value == 7:
...
def wait_for_connection():
    print("The program will wait for connection to any BCC")
    logwrite("The program will wait for connection to any BCC", 1)
    attempt_count = 0
    while 1:
        for all_files in range(0,len(csv_files)):
            try:
                page = requests.get(URL[all_files], timeout=(5,10))
            except:
                attempt_count += 1
            else:
                print("Connection is back online, returning to normal mode")
                logwrite("Connection is back online, returning to normal mode", 0)
                return
            if attempt_count / 20. == int(attempt_count / 20.):
                print("Attempts for connection:", attempt_count)
                logwrite("Attempts for connection: " + str(attempt_count), 1)
        time.sleep(4.9)
...
for line in lines:
    ...
    if "Start_up_email DISABLED" in line or "Start_up_email_DISABLED" in line:
        greet_email = False
...
if subj == "None" or subj == "\n":
    print("No subject in config file, using default value (BCC)")
    logwrite("No subject in config file, using default value (BCC)", 1)
    subj = "BCC"
if len(port) < len(IP):
...
e_switch = 0
ERROR_rate = 5                                                          #How many times the error have to appear before error email is sent.
for i in range(0, len(csv_files)):
    URL.append("http://" + str(IP[i]) + ":" + str(port[i]) + "/bcc.xml")
    devices_offline.append(0)
    print(URL[i])
    ERROR_count.append(0)
    ERROR_added.append(False)
for i in range(0, 34):
...
while 1:
    for all_files in range(0,len(csv_files)):
        ...
        try:
            for i in range(1, 17):
                if one_line_values[i] == "N/A":
                    continue
                one_line_values[i] = float(one_line_values[i].replace(",","."))
            for i in range(1, 17):
                if one_line_values[i] == "N/A":
                    continue
                if one_line_values[i] > 4.5 or one_line_values[i] == 4.08:
                    raise Exception("")
            ...
            if one_line_values[17] > one_line_values[18]:
                raise Exception("")
            if one_line_values[21] > 4.5 or one_line_values[21] == 4.08:
                raise Exception("")
            if one_line_values[22] > 4.5 or one_line_values[22] == 4.08:
                raise Exception("")
            if one_line_values[25] > 4.5:
                raise Exception("")
            if one_line_values[0] == 65.28:
                raise Exception("")
            csv_files[all_files].write(date_time())
            csv_files[all_files].write(";")
            for i in range(len(one_line_values)-1):
                csv_files[all_files].write(str(one_line_values[i]).replace(".", ",") + ";")
            csv_files[all_files].write("\n")
            csv_files[all_files].flush()
            os.fsync(csv_files[all_files].fileno())
        except:
            ...
            if one_line_values[30] != "CONNECTED":
                ...
                if (datetime.datetime.now() - e_date).seconds >= ERROR_interval * 60:
                    ...
                    error_message(3)
                if invalid_data_count >= period:
                    invalid_data_count = 0
                    logwrite("Writing informations with errors into csv file.", 2)
                    csv_files[all_files].write(date_time())
                    csv_files[all_files].write(";")
                    for i in range(len(one_line_values)):
                        csv_files[all_files].write(str(one_line_values[i]).replace(".", ",") + ";")
                    csv_files[all_files].write("\n")
                    csv_files[all_files].flush()
                    os.fsync(csv_files[all_files].fileno())
                ...
                if invalid_data_count >= period:
                    print(str(datetime.datetime.now())[:19], "Some important values from device:", IP[all_files]," are not a valid numbers.\nWriting them to file anyway.")
                    logwrite("Some important values from device: " + IP[all_files] + " are not a valid numbers", 2)
                    print("\nBCC", all_files+1, "with adress", IP[all_files])
                    print("Actual values:\nTotal voltage:           ", str(one_line_values[0]).replace(",","."), "V")
                    print("Minimum cell voltage:    ", str(one_line_values[21]).replace(",","."), "V", "at position", str(one_line_values[23]).replace(",","."))
                    print("Maximum cell voltage:    ", str(one_line_values[22]).replace(",","."), "V", "at position", str(one_line_values[24]).replace(",","."))
                    print("Voltage difference:      ", str(one_line_values[25]).replace(",","."), "V\n")
                    invalid_data_count = 0
                    logwrite("Writing informations with errors into csv file.", 2)
                    csv_files[all_files].write(date_time())
                    csv_files[all_files].write(";")
                    ...
                    e_switch = 1
            if e_switch != 0 and all_files != 0:
                all_files -= 1
            print("Wrong data, reading again")
            logwrite("Wrong input data, reading again", 2)
            if period > 10:
                temporary_short = (period - 10)
            continue
        one_line_values[22] = 4.02
        if one_line_values[21] < one_line_values[17]:
            if ERROR_count[all_files] > ERROR_rate:
                if (datetime.datetime.now() - e_date).seconds >= ERROR_interval * 60:
                ...
                ERROR_added[all_files] = True
                logwrite("Skipping Umin error", 1)
                print("Skipping Umin error")
        elif one_line_values[22] > one_line_values[18]:
            if ERROR_count[all_files] > ERROR_rate:
                if (datetime.datetime.now() - e_date).seconds >= ERROR_interval * 60:
                ...
                ERROR_added[all_files] = True
                logwrite("Skipping Umax error", 1)
                print("Skipping Umax error")
        elif one_line_values[25] > Udiff_level and Udiff_level != 0.:
            if ERROR_count[all_files] > ERROR_rate:
                if (datetime.datetime.now() - e_date).seconds >= ERROR_interval * 60:
                ...
                ERROR_added[all_files] = True
                logwrite("Skipping Udiff error", 1)
                print("Skipping Udiff error")
        elif one_line_values[21] < Umin_warning:
            if ERROR_count[all_files] > ERROR_rate:
                if (datetime.datetime.now() - e_date).seconds >= ERROR_interval * 60:
                ...
                logwrite("Skipping Umin error", 1)
                print("Skipping Umin error")
        elif one_line_values[22] > Umax_warning:
            if ERROR_count[all_files] > ERROR_rate:
                if (datetime.datetime.now() - e_date).seconds >= ERROR_interval * 60:
                ...
                logwrite("Skipping Umax error", 1)
                print("Skipping Umax error")
        if ERROR_added[all_files] == True:
            if period > 10:
                temporary_short = period - 10
            ERROR_count[all_files] += 1
        print("\nBCC", all_files+1, "with adress", IP[all_files])
        ...
        if ERROR_added[all_files] == False and ERROR_count[all_files] != 0:
            ERROR_count[all_files] = 0
            logwrite("Error counter cleared", 0)
    for i in range(0, len(csv_files)):
        ERROR_added[i] = False
    First_time = False
    date = str(datetime.datetime.now())
    if file_date != date[:10]:
        ...
        days += 1
        server = start_email()
        logwrite("Sending midnight email", 0)
        if server != 0:
            logwrite("Email server connected", 0)
            send_to = ""
            for i in range(len(receiver_emails)):
                send_to += receiver_emails[i] + ", "
            message = MIMEMultipart()
            ...
            error_message(0, 1)
        if days >= File_length:
            logwrite("File length reached, beginning new files", 0)
