...
while 1:
    for all_files in range(0,len(csv_files)):
        ...
        try:
            ...
            if one_line_values[30] != "CONNECTED":
                ...
                if invalid_data_count >= (period / 10):
                    print("Some important values from device:", IP[all_files]," are not a valid numbers")
        ...
        if one_line_values[21] < one_line_values[17]:
            if (period <= 5 and ERROR_count[all_files] > 2) or (period > 5 and ERROR_count[all_files] > 1):
                if (datetime.datetime.now() - e_date).seconds >= ERROR_interval * 60:
        ...
        if ERROR_added[all_files] == True:
            temporary_short = int(period - 0.1 * period)
